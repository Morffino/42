...
def eL(x):
    open("logs/errors.log", "a", encoding="utf-8").write(f"[{datetime.utcnow().isoformat()}] {type(x).__name__}: {x}\n")
...
if not T or T.strip() == '':
    ...
    sys.exit(1)
if not all([T, O, L]):
    print("❌ Проверьте переменные окружения: DISCORD_TOKEN, OWNER_ID, LOG_CHANNEL_ID")
    sys.exit(1)

# ---- ЗАГРУЗКА КОНФИГА С ПРАВАМИ ----
...
try:
    with open(CONFIG_PATH, "r", encoding="utf-8") as f:
        config = json.load(f)
    ADMIN_USER_IDS = config.get("discord", {}).get("admin_user_ids", [])
    ADMIN_ROLE_IDS = config.get("discord", {}).get("admin_role_ids", [])
    print(f"✅ Загружен конфиг: {len(ADMIN_USER_IDS)} пользователей, {len(ADMIN_ROLE_IDS)} ролей")
except FileNotFoundError:
    print(f"⚠️ Файл {CONFIG_PATH} не найден. Создайте его с полями discord.admin_user_ids и discord.admin_role_ids.")
except json.JSONDecodeError:
    print(f"⚠️ Ошибка парсинга {CONFIG_PATH}. Убедитесь, что файл содержит валидный JSON.")
...
def has_allowed_role(i):
    ...
    if str(user_id) in ADMIN_USER_IDS:
        print(f"[DEBUG] Доступ РАЗРЕШЁН: ID в admin_user_ids")
        return True

    # Проверка ролей
    user_roles = [role.id for role in i.user.roles]
    print(f"[DEBUG] Роли пользователя {user_id}: {user_roles}")

    for role_id in ADMIN_ROLE_IDS:
        if int(role_id) in user_roles:
            print(f"[DEBUG] Доступ РАЗРЕШЁН: роль {role_id} найдена")
            return True

    print(f"[DEBUG] Доступ ЗАПРЕЩЁН: нет подходящей роли или ID")
    return False

async def ensure_admin(interaction: discord.Interaction) -> bool:
    """Проверяет права и отправляет ошибку, если их нет."""
...
async def la(i, a, target=None, reason=None, extra=None):
    ...
    if target:
        e.add_field(name="Цель", value=discord.utils.escape_mentions(f"{target.mention} ({target.id})"), inline=False)
    if reason:
        e.add_field(name="Причина", value=discord.utils.escape_mentions(reason), inline=False)
    if extra:
        e.add_field(name="Дополнительно", value=discord.utils.escape_mentions(extra), inline=False)
...
class MC(commands.Cog):
    ...
    async def cog_check(self, i):
        ...
        print("[DEBUG] Доступ разрешён, команда выполняется")
        return True
    ...
    async def ws(self, i, member: discord.Member):
        ...
        if uid not in w or not w[uid]:
            await i.response.send_message(f"У {member.mention} нет предупреждений.", ephemeral=True)
            return
        await i.response.defer(ephemeral=True)
    ...
    async def bl(self, i, user: discord.User, action: str):
        if not await ensure_admin(i):
            return
        # Дополнительная проверка на владельца
        if i.user.id != O:
            await i.response.send_message("❌ Только владелец.", ephemeral=True)
            return
        await i.response.defer(ephemeral=True)
        bl = lb()
        uid = str(user.id)
